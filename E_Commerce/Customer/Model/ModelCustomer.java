...
public class ModelCustomer implements IModelCustomer 
{
	public String buy(int product_Id,int givenQuantity,int givenPrice) throws SQLException
	{
		...
		if(givenPrice>=price*givenQuantity)
		{
			...
			else return "Only "+quantity+"are avilable";
		}
		else
			return "Low balance";
	}
	...
	public String cart(int product_id) throws SQLException
	{
	    ...
	    s1.execute("insert into cart values ("+product_id+",'"+Customer_name+"','"+Product_Name+"','"+Category_Name+"','"+Product_Description+"',"+price+")");  
